...
void g2o_addOdomFactor()
{
    if (keyPose3DCloud->points.size()==1)
    {
        Eigen::Affine3d T_curr_temp;
        PointTypePose6D thisPoint=keyPose6DCloud->points[keyPose3DCloud->points.size()-1];
        pcl::getTransformation(double(thisPoint.x), double(thisPoint.y), double(thisPoint.z), double(thisPoint.roll), double(thisPoint.pitch), double(thisPoint.yaw),T_curr_temp);
       
        Eigen::Isometry3d T_curr ;
        T_curr.translation() = T_curr_temp.translation();
        T_curr.linear() = T_curr_temp.rotation();
        
        //第一个顶点
         g2o::VertexSE3 *vertex(new g2o::VertexSE3());

         vertex->setId(keyPose3DCloud->points.size()-1);
        vertex->setEstimate(T_curr);
    ...
    if (keyPose3DCloud->points.size()>1)
    {
        PointTypePose6D lastPoint=keyPose6DCloud->points[keyPose3DCloud->points.size()-2];
        //PointTypePose6D thisPoint=keyPose6DCloud->points[keyPose3DCloud->points.size()-1];

        Eigen::Affine3d T_last_temp;
        Eigen::Affine3d T_curr_temp;
       
        pcl::getTransformation(double(lastPoint.x),double(lastPoint.y), double(lastPoint.z),double(lastPoint.roll),double(lastPoint.pitch), double(lastPoint.yaw),T_last_temp);
        pcl::getTransformation(arr6d[0], arr6d[1], arr6d[2], arr6d[3], arr6d[4], arr6d[5],T_curr_temp);

        //Eigen::Affine3d relative_pose_temp= T_last_temp.inverse() *T_curr_temp;
        Eigen::Isometry3d T_last;
        T_last.translation() = T_last_temp.translation();
        T_last.linear() = T_last_temp.rotation();

        Eigen::Isometry3d T_curr ;
        T_curr.translation() = T_curr_temp.translation();
        T_curr.linear() = T_curr_temp.rotation();
        //TODU
        //Eigen::Isometry3d relative_pose =relative_pose_temp.matrix();

        // Eigen::AngleAxisd rollAngle(AngleAxisd(thisPoint.roll,Vector3d::UnitX()));
        // Eigen::AngleAxisd pitchAngle(AngleAxisd(thisPoint.pitch,Vector3d::UnitY()));
        // Eigen::AngleAxisd yawAngle(AngleAxisd(thisPoint.yaw,Vector3d::UnitZ()));
        // Eigen::Matrix3d R_curr;
        // R_curr=yawAngle*pitchAngle*rollAngle;

        // rollAngle(AngleAxisd(lastPoint.roll,Vector3d::UnitX()));
        // pitchAngle(AngleAxisd(lastPoint.pitch,Vector3d::UnitY()));
        //  yawAngle(AngleAxisd(lastPoint.yaw,Vector3d::UnitZ()));
        //  Eigen::Matrix3d R_last;
        // R_last=yawAngle*pitchAngle*rollAngle;

    
        // Eigen::Vector3d t_last(thisPoint.x,thisPoint.y,thisPoint.z);
        // Eigen::Vector3d t_curr(lastPoint.x,lastPoint.y,lastPoint.z);

        // Eigen::Isometry3d T_last= Isometry3d::Identity();
        // Eigen::Isometry3d T_curr= Isometry3d::Identity();
        // T_last.translate(t_last);
        // T_last.rotate(R_last);
        // T_curr.translate(t_curr);
        // T_curr.rotate(R_curr);
        //计算边信息矩阵
        Vector6d odom_noise;
        odom_noise<<1e-4, 1e-4, 1e-4,1e-6, 1e-6, 1e-6;
        Matrix6d information_matrix=Matrix6d::Identity();
        for(int i=0;i<6;i++)
            information_matrix(i,i) /=odom_noise(i);
         Eigen::Isometry3d relative_pose=T_last.inverse()*T_curr;
        //顶点
         g2o::VertexSE3 *vertex(new g2o::VertexSE3());
        ...
        edge->setVertex(0, optimizer.vertices()[keyPose3DCloud->points.size()-2]);
        edge->setVertex(1,optimizer.vertices()[keyPose3DCloud->points.size()-1]);
...
void g2o_addLoopFactor()
{
    ...
    if(detectLoopFrameID(&loopKeyCur, &loopKeyPre)){
        ...
        Eigen::Affine3d tPre;
        PointTypePose6D thisPoint=keyPose6DCloud->points[loopKeyPre];
        pcl::getTransformation(thisPoint.x, thisPoint.y, thisPoint.z, thisPoint.roll,thisPoint.pitch, thisPoint.yaw,tPre);
        //回环边相对位姿
        Eigen::Affine3d relative_pose_temp = tPre.inverse() * tCorrect;
        Eigen::Isometry3d relative_pose;
        relative_pose.translation() = relative_pose_temp.translation();
        relative_pose.linear() = relative_pose_temp.rotation();
        //计算回环边信息矩阵
        double noiseScore = icp.getFitnessScore();
        Vector6d loop_noise;
        loop_noise<< noiseScore, noiseScore, noiseScore, noiseScore, noiseScore, noiseScore;
        Matrix6d information_matrix=Matrix6d::Identity();
        for(int i=0;i<6;i++)
            information_matrix(i,i) /=loop_noise(i);
        //添加回环边
        g2o::EdgeSE3 *edge(new g2o::EdgeSE3());
        edge->setMeasurement(relative_pose);
        edge->setInformation(information_matrix);
        edge->setVertex(0, optimizer.vertices()[loopKeyPre]);
        edge->setVertex(1,optimizer.vertices()[loopKeyCur]);
        optimizer.addEdge(edge);
        loopIndexContainer[loopKeyCur] = loopKeyPre;
        aLoopIsClosed = true;
    }
...
void g2o_updateKeypose(int i ,Eigen::Isometry3d pose)
{
    double x, y, z, roll, pitch, yaw;
    Eigen::Affine3d new_pose =pose;
    pcl::getTranslationAndEulerAngles(new_pose, x, y, z, roll, pitch, yaw);
    keyPose3DCloud->points[i].x = x;
    keyPose3DCloud->points[i].y = y;
    keyPose3DCloud->points[i].z = z;
    keyPose6DCloud->points[i].x = keyPose3DCloud->points[i].x;
    keyPose6DCloud->points[i].y = keyPose3DCloud->points[i].y;
    keyPose6DCloud->points[i].z = keyPose3DCloud->points[i].z;
    keyPose6DCloud->points[i].roll  = roll;
    keyPose6DCloud->points[i].pitch = pitch;
    keyPose6DCloud->points[i].yaw   = yaw;
    updatePath(keyPose6DCloud->points[i],i);  // 更新里程计轨迹
    pcl::getTransformation(keyPose6DCloud->points[i].x, keyPose6DCloud->points[i].y, keyPose6DCloud->points[i].z,
            keyPose6DCloud->points[i].roll, keyPose6DCloud->points[i].pitch,keyPose6DCloud->points[i].yaw,trans_map_0_curr_i);
    pcl::PointCloud<PointType>::Ptr cloud_res_i(new pcl::PointCloud<PointType>());
...
void g2o_correctPoses()
{
     ...
     {
         g2o::VertexSE3* v = dynamic_cast<g2o::VertexSE3*>(optimizer.vertex(vertex_num-1));
          Eigen::Isometry3d pose = v->estimate();
         g2o_updateKeypose(vertex_num-1 , pose);
     }
...
void mapOptimization(){
    if(isKeyFrame()){       //判断是否作为关键帧（如果原地变化很小，则不计入关键帧，相当于帧过滤）
        ...
        correctPoses();     // 更新里程计
        publishResult();
        //std::cout<<"关键帧数："<<keyFrameVector.size()<<std::endl;
    }
}
void g2o_mapOptimization()
{
        if(isKeyFrame()){       //判断是否作为关键帧（如果原地变化很小，则不计入关键帧，相当于帧过滤）
        // get x, y, z, roll, pitch, yaw
        pcl::getTranslationAndEulerAngles(T_map_0_curr,arr6d[0],arr6d[1],arr6d[2],arr6d[3],arr6d[4],arr6d[5]);
        addPose3D6D(arr6d[0],arr6d[1],arr6d[2],arr6d[3],arr6d[4],arr6d[5],timeOdom);
        keyFrameVector.push_back(*currFramePlanePtr);  // save key frame
        g2o_addOdomFactor();    // 激光里程计因子
        g2o_addLoopFactor();    // 闭环因子
        g2o_runOptimize();      // 执行优化
        g2o_correctPoses();     // 更新里程计
        publishResult();
        //std::cout<<"关键帧数："<<keyFrameVector.size()<<std::endl;
    }
}
